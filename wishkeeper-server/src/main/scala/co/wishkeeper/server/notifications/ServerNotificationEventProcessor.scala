...
class ServerNotificationEventProcessor(clientRegistry: ClientNotifier, notificationsScheduler: NotificationsScheduler) extends EventProcessor {
  override def process(event: Events.Event, userId: UUID): List[(UUID, Event)] = {
    event match {
      ...
      case _: WishReservedNotificationCreated
      => notificationsScheduler.scheduleNotification(userId, NotificationsUpdated)
      case _ =>
    }
    Nil
  }
